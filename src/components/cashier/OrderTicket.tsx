...
interface OrderTicketProps {
    order: Order | null;
    type: 'kitchen' | 'bar' | 'customer' | 'both';
    restaurantName?: string;
}

const SingleTicket: React.FC<{ order: Order, type: 'kitchen' | 'bar' | 'customer', restaurantName?: string }> = ({ order, type, restaurantName }) => {
...
export const OrderTicket: React.FC<OrderTicketProps> = ({ order, type, restaurantName }) => {
    ...
    const renderContent = () => {
        if (type === 'both') {
            return (
                <>
                    <SingleTicket order={order} type="kitchen" restaurantName={restaurantName} />
                    <div style={{ pageBreakBefore: 'always', margin: 0, padding: 0 }} />
                    <SingleTicket order={order} type="bar" restaurantName={restaurantName} />
                </>
            );
        }
        return <SingleTicket order={order} type={type} restaurantName={restaurantName} />;
    };
